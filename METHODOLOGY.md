...
| 驗證 | write-test `rc=0`/exit 0 就判「功能可用」 | rc=0 只代表「被接受/parse 過」≠ 真生效；要驗 **①安裝後真實落點**（SW-only vs HW-offload）＋**②negative control**（給個假值必須被拒，證 rc 有辨別力）。**半移植擴充**常 userspace parse 過、底層 HW-hook 缺＝rc=0 但對目標路徑（如 HW fast-path）沒用 |
